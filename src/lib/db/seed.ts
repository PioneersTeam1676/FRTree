...
for (let i = 0; i < teamsToPull.length; i++) {
    const teamId = teamsToPull[i];
    const team = await getTeamFromTBA(teamId);
    const info = team.info[0];
    
    await connection.query("INSERT INTO frclink_info (team_num, team_full_name, pfp, description, uid, primary_col, secondary_col, location) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", [
    ...
    for (const link of links) {
        await connection.query("INSERT INTO frclink_links (team_num, icon, description, url, uid, title) VALUES (?, ?, ?, ?, ?, ?)", [
